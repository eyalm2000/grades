...
export function OnboardingWelcome({ userImage, firstName, onNext }: OnboardingWelcomeProps) {
  ...
  const content = (
    ...
    >
      ...
      <motion.div 
        className="bg-gradient-to-r from-purple-50 to-blue-50 p-6 rounded-lg border border-purple-200 shadow-sm"
      ...
      >
        ...
        >
          ...
          <span className="font-semibold text-purple-800 mr-2">ברוך הבא ל-GradeWiz!</span>
        </motion.div>
      ...
      <motion.div 
        className="bg-gradient-to-r from-indigo-50 to-purple-50 p-4 rounded-lg border border-indigo-200 shadow-sm"
        variants={itemVariants}
        whileHover={{ y: -2, boxShadow: "0 8px 15px -3px rgba(0, 0, 0, 0.08)" }}
        transition={{ duration: 0.2 }}
      >
      ...
      <motion.div variants={itemVariants} className="pt-4">
        <Button 
          onClick={onNext} 
          size="lg" 
          className="bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 px-8 shadow-lg text-white transition-all duration-200 hover:scale-[1.02] hover:shadow-xl"
        >
          בואו נתחיל!
          <motion.span
            initial={{ x: 0 }}
